...
// バイナリプロトコル（転送量90%削減）- デフォルト有効
const binaryProtocol = require('./binary-protocol.js');
// デフォルトはバイナリ。JSONを使いたい場合は node server.js json
const USE_BINARY_PROTOCOL = !process.argv.includes('json') && process.env.PROTOCOL !== 'json';
...
function getUniqueColor() {
    // バイナリプロトコル: クライアントとの同期のため固定パレットを使用
    if (USE_BINARY_PROTOCOL) {
        const palette = binaryProtocol.COLOR_PALETTE;
...
function endRound() {
    ...
    setTimeout(() => {
        ...
        // バイナリプロトコルのプレイヤーマッピングをリセット（IDオーバーフロー防止）
        if (USE_BINARY_PROTOCOL) {
            binaryProtocol.resetPlayerMapping();
        }
...
function broadcast(msg) {
    ...
    if (USE_BINARY_PROTOCOL) {
        ...
        if (msg.type === 'round_start') {
            data = binaryProtocol.encodeRoundStartMessage(
                msg.mode,
                msg.world || { width: WORLD_WIDTH, height: WORLD_HEIGHT },
                msg.obstacles || []
            );
        } else if (msg.type === 'chat') {
            data = binaryProtocol.encodeChatMessage(msg.text, msg.color, msg.name);
        } else if (msg.type === 'player_death') {
            data = binaryProtocol.encodePlayerDeathMessage(msg.id, msg.reason);
        } else {
            // その他はJSONフォールバック
            data = JSON.stringify(msg);
        }
        byteLen = Buffer.isBuffer(data) ? data.length : Buffer.byteLength(data, 'utf8');
    } else {
        data = JSON.stringify(msg);
...
wss.on('connection', ws => {
    ...
    if (USE_BINARY_PROTOCOL) {
        const playerMappings = Object.values(players).map(p => ({
            id: p.id, name: p.name, emoji: p.emoji, team: p.team
        }));
        const initMsg = binaryProtocol.encodeInitMessage(
            id, color,
            { width: WORLD_WIDTH, height: WORLD_HEIGHT },
            GAME_MODES[currentModeIdx],
            obstacles,
            territoryRects,
            GRID_SIZE,
            playerMappings
...
setInterval(() => {
    ...
    // マッピングデータ（STATE_FULLで送信）
    const playerMappings = USE_BINARY_PROTOCOL ? Object.values(players).map(p => ({
        id: p.id, name: p.name, emoji: p.emoji, team: p.team
    })) : null;
    ...
    wss.clients.forEach(c => {
        ...
        const needFullSync = territoryVersion - lastVersion > 10 || lastVersion === 0;

        if (USE_BINARY_PROTOCOL) {
            // バイナリプロトコル
            // テリトリー変化(stateMsg.td)がある場合、バイナリでは差分未対応のためフル同期を送る
            // または、クライアントが古すぎる場合(needFullSync)もフル同期
            const sendFull = needFullSync || !!stateMsg.td;

            msgData = binaryProtocol.encodeStateMessage(
                cleanPlayers,
                timeRemaining,
                sendFull ? territoryRects : null,
                sendFull,
                GRID_SIZE,
                sendFull ? playerMappings : null
            );
            if (sendFull) {
        ...
        // 転送量記録
        const byteLen = USE_BINARY_PROTOCOL ? msgData.length : Buffer.byteLength(msgData, 'utf8');
        ...
        if (isFullSync && bandwidthStats.periodFullSyncs % 10 === 1 && !USE_BINARY_PROTOCOL) {
            try {
                const compressed = zlib.deflateSync(msgData);
...
initGrid();
server.listen(PORT, () => {
    console.log(`Server Grid Mode Started ${PORT}`);
    console.log(`Protocol: ${USE_BINARY_PROTOCOL ? 'BINARY (90%削減)' : 'JSON'}`);
    if (USE_BINARY_PROTOCOL) {
        console.log('[BINARY] バイナリプロトコルが有効です。クライアント側も対応が必要です。');
    }
});
...
function printRoundStats() {
    ...
    console.log(`║ 🎮 モード: ${mode.padEnd(10)} | プロトコル: ${USE_BINARY_PROTOCOL ? 'BINARY' : 'JSON'}`);
    console.log(`║ 👥 接続数: ${playerCount}人 (アクティブ: ${activePlayerCount}人)`);
    console.log('╠══════════════════════════════════════════════════════════════════════════════╣');
