...
setInterval(() => {
    ...
    Object.values(players).forEach(p => {
        ...
        // Bot AI (High Precision Utility Model)
        if (p.isBot) {
            const range = 120; // Vision range
            const trailLen = p.trail.length;
            const currentAng = Math.atan2(p.dy, p.dx);

            // Candidate Angles (16 directions)
            let bestAng = currentAng;
            let maxScore = -Infinity;

            for (let i = 0; i < 16; i++) {
                const ang = i * (Math.PI / 8); // 0, 22.5, 45...
                let score = 0;

                // 1. Safety Check (Raycast)
                // Check immediate danger (short range) and planning (long range)
                let hitSelf = false;
                let hitWall = false;
                let hitObs = false;

                // Multi-step raycast for this angle
                ...
                for (let dist of steps) {
                    ...
                    // Wall
                    if (lx < 20 || lx >= WORLD_WIDTH - 20 || ly < 20 || ly >= WORLD_HEIGHT - 20) { hitWall = true; break; }

                    const lgx = Math.floor(lx / GRID_SIZE);
                    const lgy = Math.floor(ly / GRID_SIZE);

                    // Obstacle
                    if (lgx >= 0 && lgx < GRID_COLS && lgy >= 0 && lgy < GRID_ROWS) {
                        if (worldGrid[lgy][lgx] === 'obstacle') { hitObs = true; break; }

                        const cellOwner = worldGrid[lgy][lgx];
                        const isHome = (cellOwner === p.id) || (p.team && players[cellOwner] && players[cellOwner].team === p.team);

                        if (isHome) { hitHome = true; hitEmpty = false; }
                        else if (cellOwner) {
                            // Enemy territory?
                        }
                    }
                    ...
                    if (p.trail.length > 15) {
                        const skip = 10;
                        for (let k = 0; k < p.trail.length - skip; k++) {
                            if ((lx - p.trail[k].x) ** 2 + (ly - p.trail[k].y) ** 2 < 1600) { hitSelf = true; break; }
                        }
                    }
                    if (hitSelf) break;
                    ...
                    Object.values(players).forEach(e => {
                        if (e.id !== p.id && e.state === 'active') {
                            if ((lx - e.x) ** 2 + (ly - e.y) ** 2 < 2500) hitEnemy = true; // Avoid Head-on
                            // Trail
                            if (e.trail.length > 0) {
                                for (let t of e.trail) {
                                    if ((lx - t.x) ** 2 + (ly - t.y) ** 2 < 900) hitEnemyTrail = true;
                                }
                            }
                        }
                    });
                ...
                if (hitWall || hitObs || hitSelf) score = -10000;
                else {
                    if (hitEnemy) score -= 500; // Avoid collision risk
                    if (hitEnemyTrail) score += 300; // Kill opportunity
                    if (hitHome) {
                        // Return logic: Urgency scales with trail length
                        score += (trailLen * trailLen) * 0.5;
                        if (trailLen < 10) score -= 50; // Discourage early return
                    } else if (hitEmpty) {
                        // Expand logic
                        score += 50;
                        // Penalty for very long trail (Don't expand forever)
                        if (trailLen > 50) score -= 200;
                    }

                    // Consistency Bonus (momentum)
                    const diff = Math.abs(ang - currentAng);
                    const diffNorm = diff > Math.PI ? 2 * Math.PI - diff : diff;
                    score -= diffNorm * 10; // Penalty for turning
                }
            ...
            // Smooth turn towards best angle
            const desiredAngle = bestAng;
            // Immediate set (Bots have perfect reflex)
            p.dx = Math.cos(desiredAngle);
            p.dy = Math.sin(desiredAngle);
        }
...
function killPlayer(id, reason, skipWipe = false) {
    ...
    if (p && p.state === 'active') {
        ...
        // AFK Logic
        if (!p.isBot && !p.hasMovedSinceSpawn) {
            p.afkDeaths++;
        ...
        setTimeout(() => { if (players[id]) respawnPlayer(players[id]); }, p.isBot ? 1000 : RESPAWN_TIME * 1000);
    }
}
...
function endRound() {
    ...
    setTimeout(() => {
        ...
        const activePlayers = Object.values(players).filter(p => p.isBot || (p.ws && p.ws.readyState === WebSocket.OPEN));
...
setInterval(() => {
    if (!roundActive) return;
    const now = Date.now();
    const cleanPlayers = Object.values(players)
...
function createBot() {
    ...
    const color = getUniqueColor();
    const mode = GAME_MODES[currentModeIdx];
    // In Solo, team must be empty. In Team, 'bot'.
    const team = (mode === 'TEAM') ? 'bot' : '';
    const name = (mode === 'TEAM') ? '[bot] bot' : 'bot';

    players[id] = {
        id, color, emoji: '🤖', name: name,
        x: 0, y: 0, dx: 0, dy: 0,
        gridTrail: [], trail: [],
        score: 0, state: 'waiting',
        ws: null, invulnerableUntil: 0,
        afkDeaths: 0, hasMovedSinceSpawn: true,
        originalColor: color, requestedTeam: 'bot', kills: 0, team: team,
        isBot: true,
        aiData: { changeDirTimer: 0 }
    };
    respawnPlayer(players[id], true);
}

// Spawn Bots
for (let i = 0; i < 1; i++) createBot();
